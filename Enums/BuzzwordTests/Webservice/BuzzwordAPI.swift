...
class BuzzwordAPITest: XCTestCase {
    ...
    override func setUp() {
        ...
        self.sut = BuzzwordAPI(session: self.session)
    }
    ...
    func testCreateBuzzword_NetworkError() {
        session.fake(nil, response: nil, error: NSError(domain: NSURLErrorDomain, code: -1001, userInfo: [NSLocalizedDescriptionKey:"Network Error"]))
        let exp = self.expectationWithDescription("call completion")
        sut.createBuzzword("Buzzword") { result in
            switch result {
    ...
    func testCreateBuzzword_RequestError() {
        ...
        sut.createBuzzword("Buzzword") { result in
            ...
            case .Failure(let errorMessage):
                exp.fulfill()
                XCTAssertEqual(errorMessage, "Request Error")
            default:
                XCTFail("should not been invoked")
            }
        }
